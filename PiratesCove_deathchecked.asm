...
skeleton_dead_already:		.asciiz "Your crew has survived!
...
check_health_death:
	...
	#decrease number of living crew
	sub $s7, $s7, 1
...
check_health_cured:
	#check for curing of already dead character
	la $t4, crew_health	#pull crew health array address
	sll $t3, $t3, 2		#get the offset for the correct array location
	add $t4, $t4, $t3	#get the array location in question (add offset)
	lw $t5, ($t4)		#pull the value stored
...
skeleton_battle_death:
	...
	add $t1, $t1, $t0
	lw $t2, ($t1)
	beq $t2, -1, skeleton_killed_deadman

	#set crew member as dead in crew_health
	li $t3, -1
	sw $t3, ($t1)
	
	#print message for crew death
...
skeleton_killed_deadman:
	#print message for no crew death & jump to exit
	la $a0, skeleton_dead_already
	li $v0, 4
	syscall
